...
enum AccessPolicy {
    static func canCreateMatch(_ user: User?) -> Bool {
        ...
        return user.isAdmin || user.globalRole == .player || user.isCoachActive
    }
    ...
    static func canCreateTournament(_ user: User?) -> Bool {
        guard let user else { return false }
        return user.isAdmin || user.isOrganizerActive
    }

    static func canEditTournament(_ user: User?, _ tournament: Tournament) -> Bool {
        hasAdminOrSubscribedTournamentOrganiserAccess(user, ownerId: tournament.ownerId, organiserIds: tournament.organiserIds)
    }

    static func canManageTournamentTeams(_ user: User?, _ tournament: Tournament) -> Bool {
        hasAdminOrSubscribedTournamentOrganiserAccess(user, ownerId: tournament.ownerId, organiserIds: tournament.organiserIds)
    }

    static func canCreateTournamentMatch(_ user: User?, _ tournament: Tournament) -> Bool {
        hasAdminOrSubscribedTournamentOrganiserAccess(user, ownerId: tournament.ownerId, organiserIds: tournament.organiserIds)
    }
    ...
    static func canCreateCoachSession(_ user: User?) -> Bool {
        guard let user else { return false }
        return user.isAdmin || user.isCoachActive
    }

    static func canEditCoachSession(_ user: User?, _ session: CoachSessionAccessTarget) -> Bool {
        hasAdminOrOrganiserAccess(user, ownerId: session.ownerId, organiserIds: session.organiserIds)
    }
    ...
    private static func hasAdminOrSubscribedTournamentOrganiserAccess(_ user: User?, ownerId: UUID, organiserIds: [UUID]) -> Bool {
        guard let user else { return false }
        if user.isAdmin { return true }
        guard user.isOrganizerActive else { return false }
        let ids = Set(organiserIds).union([ownerId])
        return ids.contains(user.id)
    }
}
